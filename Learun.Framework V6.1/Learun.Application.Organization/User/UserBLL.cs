...
namespace Learun.Application.Organization
{
    ...
    public class UserBLL : UserIBLL
    {
        ...
        /// <param name="domainlogin">是否域账户登录</param>
        /// <returns></returns>
        public UserEntity CheckLogin(string account, string password, string domainlogin)
        {
            try
            {
                ...
                if (userEntity.F_EnabledMark == 1)
                {
                    ...
                    {
                        #region 域身份认证
                        string adserver = Config.GetValue("adServer");
                        string adPath = "LDAP://" + adserver;
                        LdapAuthentication adAuth = new LdapAuthentication(adPath);
                        if (true == adAuth.IsAuthenticated(adserver, account, password))
                        {
